...
function BrowserInfo(win) {
    ...
    do {
        ...
        if(i >= 0) {
         ...
         /*  Handle IE 11 User Agent Strings:
        * Samples: 
        * Windows 7: 'Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko'
        * 64-bit Windows 8.1 Update: 'Mozilla/5.0 (Windows NT 6.3; Win64, x64; Trident/7.0; Touch; rv:11.0) like Gecko'
        * Other variants are possible.               
        * The unique property is 'Trident', the name of the IE rendering engine. 
        * Look for that, then locate 'rv:' to initialize the version number.
        */
        var i = ua.indexOf("Trident");
        if(i >= 0) {
        	versionFlag = "rv:";
            this.isIE    = true;
            this.browser = "MSIE";
            this.version = parseFloat(ua.substring(versionFlag + 1, versionFlag + 4));
            break;
        }
        i = ua.indexOf("Opera/");
        if(i >= 0) {
...
function BrowserInfo_getAnchor(id) {
    if(this.isNN || (this.isOP && this.version >= 6.0)) {
        var obj = this.m_window.document.anchors[id];
        return typeof(obj) == "undefined" ? null : obj;
    }
    else {
...
function TreeMenu_generate(method) {
    ...
    if(!this.m_reload) {
        ...
        // MK (D032118): Changed
        this.g_item_id    = '<tr><td ' + this.g_itemHeight + ' id="' + this.m_nameTd;
...
function TreeMenu_g_itemLTR(item, state) {
    ...
    // MK (D032118): Changed
    html += this.g_item_id + index + '" class="treeLevel' + item.level + '" bgcolor="' + bgcolor + '"\n>';
...
function TreeMenu_i_mark(index) {
    ...
    if(this.browser.isMarkTypeBgcolor) {
        if(marked != -1 && typeof(markedItem) != 'undefined') {
            tag = this.browser.getElementById(this.m_nameTd + marked);
            if(tag != null) {
                tag.style.backgroundColor = markedItem.m_bgcolor;
            }
        }

        tag = this.browser.getElementById(this.m_nameTd + index);
        if(tag != null) {
