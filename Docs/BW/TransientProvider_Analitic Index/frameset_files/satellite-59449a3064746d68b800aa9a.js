...
/************************** CONFIG SECTION **************************/
var sap_s_account = (typeof(window.sap_s_account)=='string')?window.sap_s_account:'saphelpportaldev';

/* Get page/server info */
var sap_s = new Object();
sap_s.lastFileUpdate = 'DTM:2017.09.27'; // ACR.josh
sap_s.siteCode = 'help';
sap_s.linkInternalFilters = 'javascript:,help.sap.com,.hana.ondemand.com';
...
sap_s.dynamicAccountList = 'saphelpportal,sapglobal=help.sap.com';
...
console.log('DTM: _getReferrer: ref (1): ',ref);

  if (ref && ref[1]) {
    ref = unescape(ref[1]).split('|');
console.log('DTM: _getReferrer: ref (2): ',ref);

    if (+ref[0] === 0 && ref[1]) {
      document.cookie = 's_referrer=' + escape('1|' + ref[1]) + ';path=/;domain=' + location.hostname.split('.').slice(-2).join('.');
console.log('DTM: _getReferrer: ref (3): ',ref);

      return ref[1];
    }
  }
console.log('DTM: _getReferrer: EOF (document.referrer): ',document.referrer);
...
} // end setDemandBaseVars

function local_sap_s() {
  console.log('DTM: local_sap_s: sap_s.referrer (1): ',sap_s.referrer);
  if ( sap_s.isTrackCall && sap_s.isTrackCall.T ) {
    sap_s.referrer = sap_s._getReferrer();
  }
  console.log('DTM: local_sap_s: sap_s.referrer (2): ',sap_s.referrer);
...
/** stuff to mimic omni.epx **/
  ...
  if (!sap_s.c_rr('session')) {
    ...
    sap_s.prop2 = (''+(sap_s.prop2||'us-en')).toLowerCase();
    sap_s.prop5 = 'glo';
    sap_s.linkDownloadFileTypes = 'exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,rar,sda';

    /* page name */
    if (!sap_s.pageName || sap_s.pageName == '') { 
      sap_s.pageName = (sap_s.server + ':' + sap_s.thisPathName).toLowerCase().substring(0, 254);
    }    
    // bandaid: add site prefix if necessary because it is not done on some pages
    var _pn = new RegExp('^('+sap_s.server+':)?');  
    sap_s.pageName=sap_s.pageName.replace(_pn,sap_s.server+':');
