...
sap_s.lastUpdateGlobal = 'DTM_AM:2017.12.18';  // Last updater: ACR.JD
...
sap_s.linkInternalFilters=sap_s.linkInternalFilters.replace(/(#|javascript:)(,|$)/g,'').replace(/^,|,$/g,'');
...
/* Auto Exit Link tracking */
    ...
    sap_s.prop50 = sap_s.server + ":" + sap_s.lastFileUpdate + "|gl:" + sap_s.lastUpdateGlobal +sap_s._aaVersion + sap_s._mboxVersion +sap_s._mcidVersion;
...
/* form confirmation */
  if(sap_s.inList('event4',sap_s.events,',',':') && sap_s.eVar50){
...
/* cross-site campaigns */
  sap_s.eVar39 = sap_s.Util.getQueryParam('xsite');
  sap_s.eVar39=sap_s.getValOnce(sap_s.eVar39,'v39',0);
...
/* internal search */
  if(sap_s.prop11){
    ...
    if (!sap_s.prop13) {
      sap_s.prop13=document.referrer;
...
/* var dupes */
  ...
  sap_s.transactionID = sap_s.purchaseID;
...
if (!sap_s.c_r('s_ttc')) {
  sap_s.s_ttc = (typeof(window.omni_ttc) != 'undefined') ? window.omni_ttc : s_getCurrentUnixTimeStamp();
  sap_s.expDate=new Date();
  sap_s.expDate.setDate(sap_s.expDate.getDate()+365);
  sap_s.c_w('s_ttc',sap_s.s_ttc,sap_s.expDate);
}
...
if (sap_s.inList('event8',sap_s.events,',',':')) {
  sap_s.expDate=new Date();
  sap_s.expDate.setDate(sap_s.expDate.getDate()+365);
  sap_s.c_w('s_ttc','-',sap_s.expDate);
}

// when OL1 event happens, record how long it took user to convert.
// d:m:h if new user and first conversion, "repeat" if existing user
// or subsequent OL1 events 
if(sap_s.inList('event4',sap_s.events,',',':')){
  sap_s.s_ttc = sap_s.c_r('s_ttc');
  if (sap_s.s_ttc == '-') {
    sap_s.eVar4 = 'repeat';
  } else {
    sap_s.cTimeStamp = s_getCurrentUnixTimeStamp();
    sap_s.sTimeStamp = (sap_s.c_r('s_ttc')) ? sap_s.c_r('s_ttc') : sap_s.cTimeStamp;
    sap_s.eVar4 = s_getTimeToConvert(sap_s.sTimeStamp,sap_s.cTimeStamp);
  }
  sap_s.expDate=new Date();
  sap_s.expDate.setDate(sap_s.expDate.getDate()+365);
  sap_s.c_w('s_ttc','-',sap_s.expDate);
...
/* bandaid: suppress engagement tracking for all video events except (re)play */
  ...
  sap_s.events=sap_s.removeEvent(sap_s.events,'event25');
  sap_s.prop7=sap_s.prop18=sap_s.prop14=sap_s.prop15=sap_s.prop16=sap_s.prop25=sap_s.prop38=sap_s.eVar38=sap_s.eVar8='';
...
/* every call */
  ...
  if (sap_s.linkType) {
    sap_s.trackEveryCall = ['eVar92','contextData.s_dmdbase','contextData.s_dmdbase_custom','contextData.s_dmdbase_downstream','prop73','prop72','plugins','eVar3','channel','prop5','prop50','prop38','eVar38','transactionID','prop50','visitorID','prop48','eVar48','server','prop9','eVar9','prop7','eVar7','eVar46','eVar8','eVar20','eVar9','eVar10','eVar15','prop16','eVar76','prop69'];
    try{
 ...
 */
sap_s.trackData = function (params,tt,ld,navigate) {
 ...
 */
sap_s.track = function (params,tt,ld,navigate) {
  var s=this;
  // legacy
    s.td_tl=false;
    s.td_t=false;
...
if(!sap_s.__ccucr)
{
    ...
    function c_r(k)
    {
        ...
        i = c.indexOf(' ' + k + '='); c = i < 0 ? s.c_rr('sap_s_sess') : c;
    ...
    function c_rspers()
    {
        var s = this, cv = s.c_rr("sap_s_pers"), date = new Date().getTime(), expd = null, cvarr = [], vcv = "";
...
if(!sap_s.__ccucw)
{
    ...
    function c_w(k, v, e)
    {
        var s = this, d = new Date, ht = 0, pn = 'sap_s_pers', sn = 'sap_s_sess', pc = 0, sc = 0, pv, sv, c, i, t, f;
